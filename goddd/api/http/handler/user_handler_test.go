...
func TestUserHandler_CreateUsers_InvalidPayload(t *testing.T) {
	svc := service_mocks.NewMockUserService(t)
	ctrl := handler.NewUserHandler(svc)
...
func TestUserHandler_UpdateUser_FailedToParseId(t *testing.T) {
	svc := service_mocks.NewMockUserService(t)
	ctrl := handler.NewUserHandler(svc)

	invalidId := "invalid-id-format"
	reqBody := `{"unknown_key":"john"}`
	req := httptest.NewRequest(http.MethodPost, "/users"+invalidId, strings.NewReader(reqBody))
	rec := httptest.NewRecorder()
	...
	assert.Equal(t, "INVALID_INPUT", res["code"])
	assert.Equal(t, "invalid input: user Id must be of type uuid", res["message"])
}

func TestUserHandler_UpdateUser_InvalidPayload(t *testing.T) {
	svc := service_mocks.NewMockUserService(t)
	ctrl := handler.NewUserHandler(svc)
	...
	reqBody := `{"unknown_key":"john"}`
	req := httptest.NewRequest(http.MethodPost, "/users"+id.String(), strings.NewReader(reqBody))
...
func TestUserHandler_UpdateUser_ServiceError(t *testing.T) {
	...
	mockErr := gofakeit.Error()
	svc.EXPECT().UpdateUser(mock.Anything, mock.Anything).Return(models.User{}, mockErr)
...
func TestUserHandler_UpdateUser_Success(t *testing.T) {
	...
	// mock service behavior
	svc.EXPECT().UpdateUser(mock.Anything, updateUserReq).Return(expUser, nil)
